...
# 定义功能访问权限（与experiment_main.py严格一致）
ACCESS_CODES = {
    "00": [],  # 对照组：无特殊功能
...
def create_interface():
    ...
    with gr.Blocks(title="医疗实验系统 - 多进程版本") as interface:
            ...
            with gr.Row():
                ...
                ">
                    ...
                    ">
                        ...
                        <button id="close-welcome" onclick="
                            var btn = document.getElementById('close-welcome-gradio');
                            if (btn) {
                                btn.click();
                            }
                            return false;
        ...
        with gr.Row():
            ...
            with gr.Column(scale=2):
                with gr.Tabs() as tabs:
                    ...
                    # 新增01组独立决策记录模块
                    with gr.Tab("01组决策功能", visible=False) as group_01_tab:
                        gr.Markdown("### 01组决策功能")
                        ...
                        with gr.Column(visible=False) as accept_box_01:
                            gr.Markdown("### 请选择接受原因（可多选）")
                            gr.Markdown("*提示：可以选择多个原因*")
                            accept_reasons_01 = gr.CheckboxGroup(
                                choices=["证据完整可验证", "逻辑自洽", "判断确定性高"],
                                label=""
                            )
                            confirm_accept_btn_01 = gr.Button("确认", variant="secondary")
                        
                        with gr.Column(visible=False) as reject_box_01:
                            gr.Markdown("### 请选择拒绝原因（可多选）")
                            gr.Markdown("*提示：可以选择多个原因*")
                            reject_reasons_01 = gr.CheckboxGroup(
                                choices=["证据不完整或不可验证", "逻辑不通", "判断确定性低"],
                                label=""
                            )
                    ...
                    with gr.Tab("决策记录", visible=False) as record_tab:
                        ...
                        history = gr.Dataframe(
                            headers=['病例名称', '状态', '时间', '原因'],
                            label="决策历史",
                            value=[],
                            interactive=False
                        )
                        
                        with gr.Column(visible=False) as accept_box:
                            gr.Markdown("### 请选择接受原因（可多选）")
                            gr.Markdown("*提示：可以选择多个原因*")
                            accept_reasons = gr.CheckboxGroup(
                                choices=["证据完整可验证", "逻辑自洽", "判断确定性高"],
                                label=""
                            )
                            confirm_accept_btn = gr.Button("确认", variant="secondary")
                        
                        with gr.Column(visible=False) as reject_box:
                            gr.Markdown("### 请选择拒绝原因（可多选）")
                            gr.Markdown("*提示：可以选择多个原因*")
                            reject_reasons = gr.CheckboxGroup(
                                choices=["证据不完整或不可验证", "逻辑不通", "判断确定性低"],
                                label=""
                            )
        ...
        def validate_access(code):
            """严格按照experiment_main.py的分组和功能分配"""
            try:
                # 提取组别代码（前两位数决定功能分配）
                group_code = code[:2] if len(code) >= 2 else code
                uid = code
                
                # 获取权限
                permissions = ACCESS_CODES.get(group_code, [])
                
                logger.info(f"输入的访问码: {code}")
                logger.info(f"提取的组别代码: {group_code}")
                logger.info(f"用户ID: {uid}, 访问权限: {permissions}")
                
                if permissions:
                    # 权限验证成功
                    auth_message = f"访问码验证成功！用户ID: {uid}，权限: {', '.join(permissions)}"
                    
                    # 强制隐藏弹窗
                    hide_modal = gr.update(visible=False)
                    
                    # 严格按照experiment_main.py的功能分配显示选项卡
                    speech_visible = "speech" in permissions
                    reasoning_visible = "reasoning" in permissions
                    record_visible = "reasoning_record" in permissions or "simple_record" in permissions
                    # 音频播放：对照组00、03组、04组需要显示（没有speech但有reasoning，或完全没有权限）
                    audio_play_visible = (len(permissions) == 0) or ("reasoning" in permissions and "speech" not in permissions)
                    group_01_visible = group_code == "01"  # 01组专用选项卡
                    
                    # 结论部分：01组可见（用于获取病例结论）
                    conclusion_section_visible = group_code == "01"
                    
                    return (
                        auth_message,  # auth_status
                        gr.update(visible=speech_visible),  # speech_tab
                        gr.update(visible=reasoning_visible),  # reasoning_tab
                        gr.update(visible=record_visible),  # record_tab
                        gr.update(visible=audio_play_visible),  # audio_play_tab
                        gr.update(visible=group_01_visible),  # group_01_tab
                        hide_modal,  # welcome_modal
                        uid,  # current_uid
                        conclusion_section_visible  # conclusion_section_visible
                    )
                else:
                    # 权限验证失败或对照组
                    if group_code == "00":
                        auth_message = f"访问码验证成功！用户ID: {uid}，权限: 对照组（无特殊功能）"
                        return (
                            auth_message,  # auth_status
                            gr.update(visible=False),  # speech_tab
                            gr.update(visible=False),  # reasoning_tab
                            gr.update(visible=False),  # record_tab
                            gr.update(visible=True),  # audio_play_tab - 对照组只显示音频播放
                            gr.update(visible=False),  # group_01_tab
                            gr.update(visible=False),  # welcome_modal
                            uid,  # current_uid
                            False  # conclusion_section_visible
                        )
                    else:
                        auth_message = f"访问码无效！请输入正确的访问码。"
                        return (
                            auth_message,  # auth_status
                            gr.update(visible=False),  # speech_tab
                            gr.update(visible=False),  # reasoning_tab
                            gr.update(visible=False),  # record_tab
                            gr.update(False),  # audio_play_tab
                            gr.update(visible=False),  # group_01_tab
                            gr.update(visible=True),  # welcome_modal - 保持显示
                            "",  # current_uid
                            False  # conclusion_section_visible
                        )
            except Exception as e:
                logger.error(f"访问验证失败: {str(e)}")
                return (
                    f"验证失败: {str(e)}",  # auth_status
                    gr.update(visible=False),  # speech_tab
                    gr.update(visible=False),  # reasoning_tab
                    gr.update(visible=False),  # record_tab
                    gr.update(False),  # audio_play_tab
                    gr.update(visible=False),  # group_01_tab
                    gr.update(visible=True),  # welcome_modal
                    "",  # current_uid
                    False  # conclusion_section_visible
                )
        ...
        access_code.submit(
            fn=validate_access,
            inputs=access_code,
            outputs=[auth_status, speech_tab, reasoning_tab, record_tab, audio_play_tab, group_01_tab, welcome_modal, current_uid, conclusion_section_visible]
        )
        ...
        def on_confirm_accept(reason, text, result, uid):
            """处理确认接受分析"""
            # reason现在是列表，需要转换为字符串
            ...
            logger.info(f"on_confirm_accept被调用，uid={uid}, group_code={group_code}, features={features}, reason={reason_str}")
            # 获取当前病例名称
            case_name = experiment.current_case if experiment.current_case else "未知"
            if "simple_record" in features:
                logger.info("使用simple_record保存")
                # 演示版本：直接返回预准备的结果
                simple_result = experiment.generate_medical_reasoning(text)
                logger.info(f"保存simple_record: 接受, {reason_str} - {simple_result[:100]}...")
            elif "reasoning_record" in features:
                logger.info("使用reasoning_record保存")
                if result:
                    logger.info(f"保存reasoning_record: 接受, {reason_str} - {result[:100]}...")
                else:
                    logger.warning("result为空，无法保存reasoning_record")
            else:
                logger.warning(f"uid={uid}没有记录功能，features={features}")
            return gr.update(visible=False), gr.update(value=[[case_name, "接受", datetime.now().strftime("%Y-%m-%d %H:%M:%S"), reason_str]])
        ...
        def on_confirm_reject(reason, text, result, uid):
            ...
            group_code = uid[:2] if uid and len(uid) >= 2 else uid
            features = ACCESS_CODES.get(group_code, [])
            logger.info(f"on_confirm_reject被调用，uid={uid}, group_code={group_code}, features={features}, reason={reason_str}")
            # 获取当前病例名称
            case_name = experiment.current_case if experiment.current_case else "未知"
            if "simple_record" in features:
                logger.info("使用simple_record保存拒绝记录")
                simple_result = experiment.generate_medical_reasoning(text)
                logger.info(f"保存simple_record: 拒绝, {reason_str} - {simple_result[:100]}...")
            elif "reasoning_record" in features:
                logger.info("使用reasoning_record保存拒绝记录")
                logger.info(f"保存reasoning_record: 拒绝, {reason_str}")
            else:
                logger.warning(f"uid={uid}没有记录功能，features={features}")
            return gr.update(visible=False), gr.update(value=[[case_name, "拒绝", datetime.now().strftime("%Y-%m-%d %H:%M:%S"), reason_str]])
        ...
        confirm_accept_btn.click(
            fn=on_confirm_accept,
            inputs=[accept_reasons, input_text, output_text, current_uid],
            outputs=[accept_box, history]
        )
        ...
        def on_accept_01_show():
            """显示接受原因选择框"""
            return gr.update(visible=True)
        
        def on_confirm_accept_01(reason, text, result, uid):
            """处理01组确认接受结论"""
            # reason现在是列表，需要转换为字符串
            if not reason or len(reason) == 0:
                return gr.update(visible=True), gr.update()
            ...
            group_code = uid[:2] if uid and len(uid) >= 2 else uid
            features = ACCESS_CODES.get(group_code, [])
            logger.info(f"on_confirm_accept_01被调用，uid={uid}, group_code={group_code}, features={features}, reason={reason_str}")
            # 获取当前病例名称
            case_name = experiment.current_case if experiment.current_case else "未知"
            if "simple_record" in features:
                logger.info(f"保存simple_record: 接受, {reason_str} - {result[:100]}...")
                return gr.update(visible=False), gr.update(value=[[case_name, "接受", datetime.now().strftime("%Y-%m-%d %H:%M:%S"), reason_str]])
            else:
        ...
        def on_confirm_reject_01(reason, text, result, uid):
            ...
            group_code = uid[:2] if uid and len(uid) >= 2 else uid
            features = ACCESS_CODES.get(group_code, [])
            logger.info(f"on_confirm_reject_01被调用，uid={uid}, group_code={group_code}, features={features}, reason={reason_str}")
            # 获取当前病例名称
            case_name = experiment.current_case if experiment.current_case else "未知"
            if "simple_record" in features:
                logger.info(f"保存simple_record: 拒绝, {reason_str} - {result[:100]}...")
                return gr.update(visible=False), gr.update(value=[[case_name, "拒绝", datetime.now().strftime("%Y-%m-%d %H:%M:%S"), reason_str]])
            else:
                logger.warning(f"uid={uid}没有simple_record功能，features={features}")
                return gr.update(visible=False), gr.update()
        ...
        accept_btn_01.click(
            fn=on_accept_01_show,
            outputs=[accept_box_01]
        )
        
        confirm_accept_btn_01.click(
            fn=on_confirm_accept_01,
            inputs=[accept_reasons_01, input_text_01, output_text_01, current_uid],
            outputs=[accept_box_01, history]
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='启动实验界面')
    parser.add_argument('--share', action='store_true', help='使用Gradio的share链接（需要网络连接）')
    parser.add_argument('--server-name', type=str, default='0.0.0.0', help='服务器地址（默认0.0.0.0，允许外部访问）')
    parser.add_argument('--server-port', type=int, default=7860, help='服务器端口（默认7860）')
    args = parser.parse_args()
    
    interface = create_interface()
    
    # 如果使用frpc等内网穿透工具，不要使用share=True
    # 直接使用本地地址，通过frpc配置的地址访问
    print(f"\n{'='*60}")
    print("实验界面启动信息")
    print(f"{'='*60}")
    print(f"服务器地址: {args.server_name}")
    print(f"服务器端口: {args.server_port}")
    
    if args.server_name == '0.0.0.0':
        print("\n访问方式：")
        print(f"  1. 本地访问: http://localhost:{args.server_port}")
        print(f"  2. 局域网访问: http://<本机IP>:{args.server_port}")
        print(f"  3. 公网访问（如果有公网IP）: http://<公网IP>:{args.server_port}")
        print(f"  4. 通过frpc访问: http://<frpc配置的地址>:<frpc配置的端口>")
        print("\n注意事项：")
        print("  - 如果使用公网IP访问，请确保：")
        print("    1. Windows防火墙已开放端口", args.server_port)
        print("    2. 路由器已配置端口转发（如果是内网环境）")
        print("    3. 云服务器安全组已开放端口（如果是云服务器）")
        print("  - 如果使用frpc，请使用frpc配置的地址访问，而不是公网IP")
    else:
        print(f"\n访问地址: http://{args.server_name}:{args.server_port}")
    
    if args.share:
        print("\n⚠️  已启用Gradio share链接（需要网络连接）")
    else:
        print("\n✓  使用本地地址模式（适合配合frpc使用）")
    ...
    interface.launch(
        share=args.share,  # 默认False，避免Windows上无法创建share链接的问题
        server_name=args.server_name,  # 0.0.0.0允许外部访问
